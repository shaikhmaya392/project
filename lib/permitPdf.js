...
export async function buildPermitPdf(permit) {
  ...
  text(`PERMIT ${permit.number}`, { size: 9, f: bold, color: MUTED });
  nl(26);
  ...
  row("Client", permit.client_name);
  row("Project", permit.project_description);
  row("Location", permit.address);

  nl(14);
  text("Services", { size: 13, f: bold, color: NAVY });
  nl(20);
  permit.services.forEach((s) => {
    text(`•  ${s}`, { size: 11 });
    nl(18);
  });

...
export async function buildQuotationPdf(quotation) {
  ...
  if (logoImage) {
    const logoSize = 58;
    page.drawImage(logoImage, { x: marginX, y: headerTop - logoSize + 16, width: logoSize, height: logoSize });
  }

  y = Math.min(headerTextBottom, headerTop - 58 + 16) - 14;
  ...
  y -= 20;
  page.drawLine({ start: { x: col.price - 70, y: y + 10 }, end: { x: rightEdge, y: y + 10 }, thickness: 1, color: LINE });
  ...
  const bannerH = 26;
  page.drawRectangle({ x: marginX, y: y - bannerH + 20, width: contentWidth, height: bannerH, color: BLUE_BG, borderColor: BLUE_BORDER, borderWidth: 1 });
  textAt(`Quotation valid until ${quotation.valid_until || ""}`, marginX + 12, y - 3, { size: 10.5, f: bold, color: BLUE_TEXT });
  y -= bannerH + 30;
  ...
  const footerY = 90;
  page.drawLine({ start: { x: marginX, y: footerY + 34 }, end: { x: rightEdge, y: footerY + 34 }, thickness: 1, color: LINE });
  if (logoImage) {
